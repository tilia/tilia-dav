...
module Tilia
  module CardDav
    class ValidateFilterTest < AbstractPluginTest
      def test_filter
        data.each_with_index do |v, i|
          (input, filters, test, result, message) = v

          if result
            assert(@plugin.validate_filters(input, filters, test), "data #{i}: #{message}")
          else
            refute(@plugin.validate_filters(input, filters, test), "data #{i}: #{message}")
          end
        end
      end
